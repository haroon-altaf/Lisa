...
import pandas as pd
import re
from static import DB_STRUCTURE, URL, MONTHS, MAN_REPORT, SERV_REPORT, MAN_SECTORS, SERV_SECTORS
from typing import List, Dict, Tuple
from urllib.parse import urlparse
from utility import WebSession, DBTransaction, TemplateLogger, find_content, p_to_str, custom_table_to_df, set_private_attr, set_class_prop
...
class IsmReport:
    ...
    def _main(cls, url: str | None = None) -> Dict[str, str | List[pd.DataFrame] | None]:
        ...
        if url:
            ...
            base_url = URL.us_man_pmi if cls._report_type == 'm' else URL.us_ser_pmi
    ...
    def _transform_sections(cls, html_sections: Dict[str, Tag | ResultSet | None]) -> Dict[str, str | List[pd.DataFrame] | None]:
        ...
        if transformed_sections['title']:
            ...
            transformed_sections['month'] = getattr(MONTHS, words[0].lower().strip())
    ...
    def _load_report_table(self) -> None:
        column_map = DB_STRUCTURE[self.__class__.__name__]['report_table']['map']
        table_name = DB_STRUCTURE[self.__class__.__name__]['report_table']['name']
    ...
    def _load_rankings_table(self) -> None:
        column_map = DB_STRUCTURE[self.__class__.__name__]['rankings_table']['map']
        table_name = DB_STRUCTURE[self.__class__.__name__]['rankings_table']['name']
    ...
    def _prep_report_table(self) -> pd.DataFrame:
        if self.__class__ is ManufacturingPmi:
