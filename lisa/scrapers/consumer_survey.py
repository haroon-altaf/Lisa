...
from lisa.database_model import US_Consumers

from .utils import MONTHS

URL_INDEX = "https://www.sca.isr.umich.edu/files/tbcics.csv"
URL_COMPONENTS = "https://www.sca.isr.umich.edu/files/tbciccice.csv"
...
class ConsumerSurvey:
    ...
    def download(cls) -> ConsumerSurvey | None:
        ...
        with WebSession() as session:
            response_index = session.get(URL_INDEX)
            response_components = session.get(URL_COMPONENTS)
    ...
    def load(self) -> None:
        column_map = US_Consumers.column_map()
        table_name = US_Consumers.name()
        df = self.table
        df_cols = df.columns
    ...
    def _process_df(df1: pd.DataFrame, df2: pd.DataFrame) -> pd.DataFrame | None:
        ...
        try:
            df1["Month"] = df1["Month"].map(lambda x: MONTHS[x.upper().strip()].value)
            df2["Month"] = df2["Month"].map(lambda x: MONTHS[x.upper().strip()].value)
        except KeyError:
            logger.exception(
                f"Month name could not be converted to number for one or more columns.\n{df1.head()}\n{df2.head()}"
            )
            return None
        ...
        try:
            ...
            df2 = df2.astype(
                {"Month": "Int64", "Year": "Int64", "Current Index": "Float64", "Expected Index": "Float64"}
            )
        except ValueError:
