...
from lisa.database_model import Bonds as Bonds_Table
from lisa.database_model import Commodities as Commodities_Table
from lisa.database_model import Crypto as Crypto_Table
from lisa.database_model import Currencies as Currencies_Table
from lisa.database_model import Stock_Indices

from .utils import set_class_prop, set_private_attr
...
class TradingEconomics:
    ...
    def _split_units(df_list: list[pd.DataFrame], response_text: str) -> list[pd.DataFrame]:
        ...
        item_unit_pairs = [
            (cell[0].findChild("b").get_text().strip(), cell[0].findChild("div").get_text().strip())
            for cell in cells
            if cell
        ]
